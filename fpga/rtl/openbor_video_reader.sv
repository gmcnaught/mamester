...
module openbor_video_reader #(
    // LEAN_SCANOUT = 1 disables the hybrid OpenBOR-engine DDR handshake
    // (joystick writes, cart writes, audio ring read/poll). The video read
    // path (control word, BUF0/BUF1, RGB565) is identical either way. The
    // lean scanout core only needs to read frames the HPS producer (gmloader)
    // writes; it never writes input/audio pointers back, so the engine that
    // busy-polls those offsets has nothing to wait on.
        ...
        case (state)
            ST_IDLE: begin
                ...
                if (enable_ddr && new_frame_pending) begin
                    ...
                    state <= ST_WRITE_CART;
                else if (!LEAN_SCANOUT && cart_size_pending)
                    state <= ST_WRITE_CART_SIZE;
                else if (!LEAN_SCANOUT && audio_wake)
