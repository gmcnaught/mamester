#!/usr/bin/env python3
"""Which hardware families would a candidate engine add that nothing else covers?

Written for mame2003-plus and since reused for lrmame (0.289); the candidate
engine is whatever `--listinfo`/`--index` describes, and every number in the
report is relative to it.
...
Three inputs, all cheap to obtain and none requiring a build:
  ...
  --index     OR, for an engine with no such XML, the JSON that
              `tools/lrmame-driver-index.py` reads out of the MAME tree itself.
              Same fields, minus screen geometry — see that tool's docstring.
...
def parse_index(path):
    """-> the same list of dicts `parse_listinfo` returns.

    The index tool already emits that shape; this exists to normalise the two
    fields the XML path derives rather than stores, so one code path downstream
    cannot tell which input it was given.
    """
    with open(path, encoding="utf-8") as fh:
        games = json.load(fh)
    for g in games:
        g["family"] = norm_family(g.get("family_raw") or g.get("family"))
        g.setdefault("description", "")
        for key in ("width", "height", "refresh", "orientation", "status"):
...
def main():
    ...
    src.add_argument("--listinfo", help="MAME-format -listinfo XML")
    src.add_argument("--index", help="JSON from tools/lrmame-driver-index.py")
    ap.add_argument("--engine", default=None,
                    help="name for the engine under test in the report "
                         "(default: mame2003-plus for --listinfo, lrmame for --index)")
    ...
    if args.listinfo:
        ...
        engine = args.engine or "mame2003-plus"
    else:
        games = parse_index(args.index)
        engine = args.engine or "lrmame"
    ...
    add(f"- {engine} game entries: **{len(games)}**, "
        f"parents: **{sum(1 for g in games if not g['cloneof'])}**")
    add(f"- {engine} hardware families: **{len(by_family)}**")
    if m4a_sets:
    ...
    add(f"Families {engine} has that mame4all does not, that no MiSTer core "
        "covers, and")
    ...
    add("")
    add("# mame4all setnames -- AUTHORITATIVE, from the built binary on the device:")
    add("ssh root@192.168.20.81 'cd /media/fat/games/mame && ./mame \"*\" "
        "-sourcefile' \\")
    add("  > mame4all-sourcefile.txt     # then --mame4all-kind sourcefile")
    add("")
    add("# An offline grep of GAME(/GAMEX( macros over vendor/mame4all-pi/src/")
    add("# drivers/*.cpp is tempting but WRONG: measured against the real build it")
    add("# missed 123 setnames and invented 154 (drivers declared in source but not")
    add("# compiled in, plus macro forms the regex does not match). It moved the")
    add("# headline by under 1%, so it is a usable fallback when no device is")
    add("# available -- but say so if you use it.")
    add("")
