...
- of the 728 coverage files, **8** run on `drcbearm32` (SH-2/SH-3) and **20** on `drcbec`, the UML interpreter, because their DRC CPU has no ARM32 back-end
...
`DRC` names the dynamic-recompiler CPU the board needs. `sh` gets
`drcbearm32`; anything else gets `drcbec`, the interpreter, at roughly an
eighth of the speed — those are the rows least likely to hold 60 Hz.
